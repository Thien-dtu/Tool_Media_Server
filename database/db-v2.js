/**
 * Database Wrapper Module v2
 * For uid/uuid-based user system
 */
...
class Database {
    ...
    /**
     * Get or create user (with uid/uuid)
     */
    ...
    /**
     * Update username (for username changes)
     */
    async updateUsername(userId, newUsername, profileUrl) {
        // Mark all old usernames as not current
        await runAsync(this.db,
    ...
    async saveCursor(usernameOrUid, apiName, cursor, pagesLoaded = 0, platform = null) {
        ...
        let apiType = await getAsync(this.db,
